...
SCREENING_PATH = ROOT / "top_censoring" / "outputs" / "gmd_lis_screening_by_survey.csv"
OUTPUT_PATH = ROOT / "top_censoring" / "outputs" / "24_surveys_four_indicators_lis_comparison.csv"
...
targets = pd.read_csv(SCREENING_PATH)
targets = targets.loc[
    pd.to_numeric(targets["Welfare share above ceiling"], errors="coerce") >= 5,
    ["Survey", "Year", "Country"],
].drop_duplicates()
target_keys = set(zip(targets["Country"].str.upper(), targets["Year"].astype(int), targets["Survey"]))
...
for chunk in reader:
    ...
    matching_target = pd.Series(
        list(zip(country, year, survey)), index=chunk.index
    ).isin(target_keys)
...
for (survey, country, year), data in microdata.groupby(["Survey", "Country", "Year"], sort=True):
    ...
    result = {
        ...
        "Welfare share above ceiling (%)": 100 * np.sum(values[values > lis_ceiling] * weights[values > lis_ceiling]) / np.sum(values * weights),
    }
...
comparison = pd.DataFrame(results).merge(targets, on=["Survey", "Year", "Country"], how="right")
